...
int shm_client(int a, int b, int c)
{
    int shmid;
    key_t key;
    char *shm, *s;
...
int shm_server(int a, int b, int d)
{
    char c;
    int shmid;
    key_t key;
    ...
    if ((shmid = shmget(key, 27, 01000 | 0666)) < 0) {
        perror("shmget");
        return(1);
    }
...
void help(){
    printf("operator - Applies operation over OpenCV:CMat object allocated in Shared Memory\n\n");
    printf("Usage example:\n");
    printf("operator (-k|--key) integer (-s|--size) integer (-t|--type) string\n\n");
    printf("Options:\n");
    printf("-k or --key integer: Key of shared memory segment. Required.\n");
    printf("-s or --size integer: Size of the structure. Required.\n");
    printf("-t or --type string: Type of the structure. Required.\n");
    exit(1);
}
...
int main(int argc, char *argv[]){
    ...
    int next_option;
    const char* const short_options = "k:s:t:" ;
    ...
    while (1) {
        ...
        next_option = getopt_long (argc, argv, short_options, long_options, ((void *)0));
        ...
        switch (next_option){
            ...
            case 't' :
                opt_type=optarg;
                type=(char *)malloc(sizeof(char) * (strlen(opt_type)+1));
                strcpy(type,opt_type);
    ...
    if( opt_key==0 || opt_size==0 || opt_type==0 ){
    ...
    if (size > 0){
        shm_client(key, size, 100);
        printf("Client\n");
    }
    else{
        shm_server(key, size, 100);
        printf("Server\n");
    }
